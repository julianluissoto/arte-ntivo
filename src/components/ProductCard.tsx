        ...
        <Link href={`/products/${product.id}`} className="flex">
            <Card className="overflow-hidden transition-all duration-300 hover:shadow-xl flex flex-col group w-full">
                <CardHeader className="p-0 relative aspect-square">
                    ...
                    <div
                        className={`absolute bottom-2 right-2 rounded-full h-24 w-24 flex items-center justify-center text-center p-1 shadow-lg z-10
                        ${product.disponible === false ? 'bg-red-600 text-yellow-300' : 'bg-primary/90 text-primary-foreground'}
                        `}
                    >

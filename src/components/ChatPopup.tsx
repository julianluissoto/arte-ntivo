  ...
  return (
    <Sheet>
        <SheetTrigger asChild>
             <Button
                className={cn(
                  "bg-primary text-primary-foreground rounded-full shadow-lg hover:bg-primary/90 transition-all duration-300 ease-in-out transform hover:scale-110",
                  "h-14 flex items-center",
                  "w-14 sm:w-auto sm:px-4" // Circle on mobile, wider on desktop
                )}
        ...
        <SheetContent 
          side="bottom" 
          className="h-[85vh] p-0 border-t-4 border-primary rounded-t-2xl 
                    md:h-auto md:w-full md:max-w-md md:bottom-24 md:right-6 md:left-auto md:top-auto md:rounded-xl"
        >
            <Card className="w-full h-full flex flex-col shadow-2xl rounded-t-2xl border-0 md:rounded-xl">
                ...
                <ScrollArea className="h-full p-6" ref={scrollAreaRef}>
                    ...
                    {messages.map((msg, index) => (
                        ...
                        <div className={`rounded-lg px-4 py-3 max-w-[80%] ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                           {msg.role === 'model' ? (
                                <p className="text-sm" dangerouslySetInnerHTML={{ __html: formatResponse(msg.content) }} />
                           ) : (
